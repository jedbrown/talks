...
%   Gives algorithmic pieces to customize for different problems (No Black Box)
  ...
  \item[]<2-> \code{-fieldsplit\_0\_pc\_type ml}
...
\only<4->{
\hskip1pt\quad\ \ \code{-pc\_fieldsplit\_schur\_factorization\_type \only<4>{diag}\only<5>{lower}\only<6>{upper}\only<7>{full}}
}
