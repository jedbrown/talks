...
\begin{frame}{MPI communicators}
  ...
  \item Operations are either
    ...
    \item Collective: \code{VecNorm(), MatMult(), MatAssemblyBegin()}
    \end{itemize}
  \item Deadlock if some process doesn't participate (e.g. wrong order)
  \item Point-to-point communication between two processes e.g. \code{MatMult()}
  \item Global communication, like the reduction in \code{VecDot()}
  \end{itemize}
\end{frame}
