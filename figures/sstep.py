...
def plot_work(opts):
    ...
    ax1.loglog(xv, work_overlap(x,2,2)/3, 'go', label='$p=2, s=3$')
    #ax1.loglog(xv, y1interior, 'b-', label='$p=2$')
    ax1.loglog(xv, work_overlap(x,2,4)/5, 'b^', label='$p=2, s=5$')
    ax1.set_xlabel('dofs/process')
    ax1.legend(loc='lower right')

    ax2.set_title('memory')
    ax2.loglog(xv, memory_overlap(x,1,1), 'r-', label='$p=1$')
    ax2.loglog(xv, memory_overlap(x,2,1), 'g-', label='$p=2$')
    ...
    ax3.loglog(xv, communication_incoming(x,2,1), 'g-', label='$p=2$')
    ax3.loglog(xv, communication_incoming(x,2,3)/3, 'go', label='$p=2, s=3$')
    #ax3.loglog(xv, communication_incoming(x,2,1), 'b-', label='$p=2$')
    ax3.loglog(xv, communication_incoming(x,2,5)/5, 'b^', label='$p=2, s=5$')
