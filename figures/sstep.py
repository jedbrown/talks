...
def plot_work(opts):
    ...
    styles = ['-', '--', '-.']
    ax1.set_title('work')
    ax1.loglog(xv, y1interior, 'k-', label='$p=1$')
    ax1.loglog(xv, work_overlap(x,1,1)/2, 'rs', label='$p=1, s=2$')
    #ax1.loglog(xv, work_interior(x,2,1), 'g-', label='$p=2$')
    ax1.loglog(xv, work_overlap(x,3,2)/3, 'go', label='$p=3, s=3$')
    #ax1.loglog(xv, y1interior, 'b-', label='$p=2$')
    ax1.loglog(xv, work_overlap(x,3,4)/5, 'b^', label='$p=3, s=5$')
    ax1.set_xlabel('dofs/process')
    ax1.legend(loc='lower right')

    ax2.set_title('memory')
    ax2.loglog(xv, memory_overlap(x,1,1), 'r-', label='$p=1$')
    ax2.loglog(xv, memory_overlap(x,3,1), 'g-', label='$p=3$')
    ax2.loglog(xv, memory_overlap(x,3,2), 'go', label='$p=3, s=3$')
    ax2.loglog(xv, memory_overlap(x,3,4), 'b^', label='$p=3, s=5$')
    ax2.set_xlabel('dofs/process')

    ax3.set_title('communication')
    ax3.loglog(xv, communication_incoming(x,1,1), 'r-', label='$p=1$')
    ax3.loglog(xv, communication_incoming(x,3,1), 'g-', label='$p=3$')
    ax3.loglog(xv, communication_incoming(x,3,3)/3, 'go', label='$p=3, s=3$')
    #ax3.loglog(xv, communication_incoming(x,3,1), 'b-', label='$p=3$')
    ax3.loglog(xv, communication_incoming(x,3,5)/5, 'b^', label='$p=3, s=5$')
